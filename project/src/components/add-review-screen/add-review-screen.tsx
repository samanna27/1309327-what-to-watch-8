...
import {AppRoute, MIN_COMMENT_LENGTH, MAX_COMMENT_LENGTH} from '../../const';
...
const mapDispatchToProps = (dispatch: ThunkAppDispatch) => ({
  onCommentSubmit(comment: {rating: number, text: string}, id: number) {
    dispatch(fetchPostCommentAction(comment, id));
  },
});
...
function AddReviewScreen(props: ConnectedComponentProps):JSX.Element {
  const {film, onCommentSubmit} = props;
          ...
          >
            <div className="rating">
              <div className="rating__stars">
                {stars.map((star) => <ReviewRatingStar key={star} rating={star}/>)}
              </div>
            </div>
